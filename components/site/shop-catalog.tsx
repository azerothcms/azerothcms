...
const categories: Array<"全部" | ShopProductCategory> = [
  "全部",
  "账号服务",
  "外观收藏",
  "坐骑伙伴",
...
const accentStyles = {
  ...
  green:
    "from-emerald-300/20 via-emerald-500/8 to-transparent text-emerald-300",
}
...
export function ShopCatalog({ products }: { products: ShopProduct[] }) {
  ...
  const cart = cartSnapshot ? readMockShopCart() : {}
  const lastOrder = orderSnapshot ? readMockShopOrder() : null
  ...
  const cartTotal = products.reduce(
    (total, product) => total + (cart[product.id] ?? 0) * product.price,
    0
  )
  const restoredOrderProducts = lastOrder
    ? lastOrder.productIds
        .map((productId) =>
          products.find((product) => product.id === productId)
        )
        .filter((product): product is ShopProduct => Boolean(product))
    : []
  const restoredOrderNotice = restoredOrderProducts.length
    ? `${copy.shop.orderRestored}：${restoredOrderProducts.map((product) => product.name).join("、")} · ${lastOrder?.total.toLocaleString()} 点券`
    : ""
  ...
  function checkout() {
    ...
    const currentCartTotal = products.reduce(
      (total, product) =>
        total + (currentCart[product.id] ?? 0) * product.price,
      0
    )
  ...
  return (
    <>
      ...
      {status || restoredOrderNotice ? (
        <GlassNotice className="mt-4" tone="info">
          {status || restoredOrderNotice}
        </GlassNotice>
      ) : null}
      {visibleProducts.length ? (
        <div className="mt-6 grid gap-4 md:grid-cols-2 xl:grid-cols-4">
          {visibleProducts.map((product) => (
            <Card
              key={product.id}
              className="glass-surface group overflow-hidden p-0 transition hover:border-primary/30"
            >
              <CardHeader
                className={cn(
                  "relative min-h-44 rounded-none p-5",
                  "bg-gradient-to-br",
                  accentStyles[product.accent]
                )}
              >
                <div className="absolute inset-0 [background-image:linear-gradient(rgba(255,255,255,0.06)_1px,transparent_1px),linear-gradient(90deg,rgba(255,255,255,0.06)_1px,transparent_1px)] [background-size:32px_32px] opacity-30" />
                ...
                <p className="relative mt-14 text-xs text-muted-foreground">
                  {product.category}
                </p>
              </CardHeader>
