...
- `app/api/account/password`：登录用户验证当前密码后更新 TrinityCore SRP6 凭据。
- `app/api/account/game-accounts`：登录用户创建并关联新的 TrinityCore 游戏账号。
...
商城商品、订单、新闻和论坛已支持读取/写入 CMS 业务表，账号邮箱、密码和关联游戏账号已支持写入 TrinityCore。CMS 未初始化时商城目录仍支持 Mock fallback；真实支付、库存发放、邮件验证、文件上传、权限审计和装备/公会详情同步留待后续阶段。
